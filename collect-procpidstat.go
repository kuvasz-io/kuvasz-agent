...
func ReadPidStat() PidStat {
	...
	for i := range list {
		...
		name := strings.Replace(fields_command[2], "/", "-", -1)
		...
		//p.pid, _    = strconv.ParseUint(fields[0], 0, 64)
		//p.comm      = fields[1][1 : len(fields[1])-1]
		p.state = fields[0][0]
		...
		//pidstat[fields_command[0]+fields_command[1]] = p
...
func CollectProcPidStat() error {
	...
	re_procstat, err = regexp.Compile(RE_PROCSTAT)
	if err != nil {
		log.Error(3, "[PS] Cannot compile procstat regex %s: %s. Bug. Cannot start.", RE_PROCSTAT, err)
		os.Exit(1)
	}
